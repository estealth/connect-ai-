...
| 설정 키 | 용도 | 기본값 |
|---------|------|--------|
| `shinAi.localBrainPath` | 로컬 지식 폴더 절대 경로 | `""` (비면 `~/.connect-ai-brain`) |
| `shinAi.secondBrainRepo` | 깃허브 저장소 URL | `""` |
| `shinAi.ollamaUrl` | AI 서버 주소 | `http://127.0.0.1:11434` |
| `shinAi.defaultModel` | AI 모델 이름 | `gemma4:e2b` |
| `shinAi.requestTimeout` | AI 응답 대기 시간(초) | `300` |
...
# VS Code Settings → shinAi.localBrainPath 값 확인

# 2. 깃허브 URL이 저장되어 있는지?
# VS Code Settings → shinAi.secondBrainRepo 값 확인
...
# → shin-ai-{version}.vsix

# GitHub 릴리즈
gh release create v{version} shin-ai-{version}.vsix -t "Release v{version}"
