...
// 1. Revert utils/config.ts to basics
const configContent = `/* SHIN AI — Configuration Utilities */
...
`;
fs.writeFileSync('src/utils/config.ts', configContent, 'utf-8');

// 2. Fix sidebar-chat.ts remaining issues
let sidebar = fs.readFileSync('src/ui/sidebar-chat.ts', 'utf-8');

// Add more missing items to imports if needed
// Actually, I'll just check if they are there.

// Fix implicit any in parameters
sidebar = sidebar.replace(/\(t\)\s*=>/g, '(t: any) =>');
sidebar = sidebar.replace(/\(f\)\s*=>/g, '(f: any) =>');
sidebar = sidebar.replace(/\(m\)\s*=>/g, '(m: any) =>');
sidebar = sidebar.replace(/\(e\)\s*=>/g, '(e: any) =>');

fs.writeFileSync('src/ui/sidebar-chat.ts', sidebar, 'utf-8');
console.log('Restored utils/config.ts and fixed any types in sidebar-chat.ts');
